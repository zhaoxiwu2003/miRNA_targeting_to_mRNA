...
class MyModel(Model, ABC):
    def __init__(self):
        ...
        self.my_model = Sequential([
            ...
            layers.Dropout(0.2),
            layers.Dense(2)])
        # self.input_layer = layers.InputLayer(input_shape=None)
        # print("*"*50)
        # self.embedding_layer = layers.Embedding(input_dim=5, output_dim=5, input_length=100)
        # print("$"*50)
        # self.conv1 = layers.Conv1D(filters=320, kernel_size=12, activation='relu')
        # self.dropout1 = layers.Dropout(0.2)
        # self.max_pooling = layers.MaxPooling1D(pool_size=2)
        # self.dropout2 = layers.Dropout(0.2)
        # self.lstm = layers.LSTM(32, activation="tanh")
    ...
    def call(self, inputs, training=None):
        # h1 = self.embedding_layer(inputs)
        # h2 = self.conv1(h1)
        # h3 = self.dropout1(h2),
        # #h4 = self.max_pooling(h3)
        # h5 = self.dropout2(h3)
        # h6 = self.lstm(h5)
        # h7 = self.dropout3(h6),
        # h8 = self.dense1(h7),
